"""
Initial frequency estimation via MSE minimization.

This module implements the search for an initial frequency estimate
within the capture range of the LMS algorithm.
"""

import numpy as np
from numpy.typing import NDArray
import warnings

from ..config import Config
from .filter_output import compute_filter_output
from .mse import compute_mse


def find_initial_theta(cfg: Config) -> tuple[float, NDArray, NDArray, NDArray]:
    """
    Find initial frequency estimate via MSE minimization.
    
    Searches θ ∈ [0, π/M] to find initial estimate within capture range.
    Identifies capture range where MSE and MSE₁ are below average.
    
    Args:
        cfg: Configuration object
        
    Returns:
        Tuple of (initial_theta, mse_values, mse_first_values, theta_range):
        - initial_theta: Initial frequency estimate (radians)
        - mse_values: MSE values over theta range
        - mse_first_values: First-stage MSE values
        - theta_range: Theta values searched
        
    Example:
        >>> cfg = Config()
        >>> initial_theta, mse, mse1, theta_range = find_initial_theta(cfg)
        >>> print(f"Initial theta: {initial_theta:.4f} rad")
    """
    # Extract parameters
    num_points = cfg.lms.num_theta_points
    num_subfilters = cfg.filter.num_subfilters
    
    # Define search range: θ ∈ [0, π/M]
    theta_range = np.linspace(0, np.pi / num_subfilters, num_points)
    
    # Initialize MSE arrays
    mse_values = np.zeros(num_points)
    mse_first_values = np.zeros(num_points)
    running_average = 0.0
    
    # Compute MSE for each theta value
    for k in range(num_points):
        theta = theta_range[k]
        
        # Compute filter output
        y = compute_filter_output(theta, cfg)
        
        # Compute MSE values
        mse_values[k], mse_first_values[k] = compute_mse(y, cfg)
        
        # Update running average
        running_average += mse_values[k] / num_points
    
    # Find global minimum
    min_mse = np.min(mse_values)
    
    # Identify capture range
    # Points where both MSE curves are below average and near minimum
    tolerance_avg = 0.0001
    tolerance_min = 0.2
    
    in_capture_range = (
        (mse_first_values - running_average < tolerance_avg) &
        (mse_values - running_average < tolerance_avg) &
        (mse_values - min_mse < tolerance_min)
    )
    
    capture_thetas = theta_range[in_capture_range]
    
    # Return initial estimate
    if len(capture_thetas) == 0:
        # Fallback: use theta at minimum MSE
        min_idx = np.argmin(mse_values)
        initial_theta = theta_range[min_idx]
        warnings.warn("Capture range not found. Using minimum MSE point.")
    else:
        initial_theta = capture_thetas[0]
    
    return initial_theta, mse_values, mse_first_values, theta_range
