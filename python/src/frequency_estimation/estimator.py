...
class FrequencyEstimator:
    ...
    def __init__(self, config: Optional[Config] = None):
        ...
        self.signal_generator = SignalGenerator(self.config.signal)
    ...
    def find_initial_theta(
        ...
        for k in range(num_points):
            theta = theta_range[k]
            outputs = self.filter_bank.process(signal, theta)
            mse_values[k], mse_first_values[k] = self._compute_mse_from_outputs(outputs)
    ...
    def estimate(self, verbose: bool = True) -> EstimationResult:
        ...
        # Generate signal once at the start
        ...
        initial_theta, mse_values, mse_first_values, theta_range = self.find_initial_theta(signal)
        ...
        final_theta, theta_history = self.run_lms(initial_theta, signal)
